...
#				step_y = GRAPH_SIZE_Y / max(1.0 / max(Benchmark.time_frame[i].min(),0.001), MAX_FPS)
	...
	find_node("Low").set_text(
		"LOW GRAPHICS - " + str(stepify(low_points,0.1))  +" fps\n"+
		"3x3 MAP - " + str(stepify(Benchmark.stages_frames_per_second[0],0.1)) + " fps\n"+
		"10x10 MAP - " + str(stepify(Benchmark.stages_frames_per_second[1],0.1)) + " fps\n"+
		"30x30 MAP - " + str(stepify(Benchmark.stages_frames_per_second[2],0.1)) + " fps"
	)
	...
	find_node("High").set_text(
		"HIGH GRAPHICS - " + str(stepify(high_points,0.1))  +" fps\n"+
		"3x3 MAP - " + str(stepify(Benchmark.stages_frames_per_second[3],0.1)) + " fps\n"+
		"10x10 MAP - " + str(stepify(Benchmark.stages_frames_per_second[4],0.1)) + " fps\n"+
		"30x30 MAP - " + str(stepify(Benchmark.stages_frames_per_second[5],0.1)) + " fps"
	)
