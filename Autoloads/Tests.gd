...
func map_test() -> void:
	...
	map = MapCreator.generate_partial_map(Vector2(13,13),75)
	assert(check_integration_of_map(map,13,13))
	map.queue_free()
	map = MapCreator.generate_partial_map(Vector2(9,9),75)
	assert(check_integration_of_map(map,9,9))
	map.queue_free()
	map = MapCreator.generate_partial_map(Vector2(15,3),75)
	assert(check_integration_of_map(map,15,3))
	map.queue_free()
	map = MapCreator.generate_full_map(Vector2(15,3))
	assert(check_integration_of_map(map,15,3))
	map.queue_free()
	map = MapCreator.generate_full_map(Vector2(13,13))
	assert(check_integration_of_map(map,13,13))
	map.queue_free()
...
func check_integration_of_map(var map : Spatial, var x, var y) -> bool:
